# Tutorial plan: *Compiled Reasoning with modenexus*
...
| 02c | `first_circuit.png` | real compiled fragment (≤25 nodes) | render FDCircuit via graphviz (small helper to write DOT — *needs a `circuit_to_dot` utility, add to `modenexus/viz.py`*) |
...
New library utility required: **`modenexus/viz.py`** with
...
1. `modenexus/viz.py` + image generator skeleton (half day).
