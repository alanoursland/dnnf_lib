...
- **Marginal MAP over modes.** ✅ *Done:* `SystemModel.compile()` now
  ...
  verified, and `CompiledSystem.map_diagnoses()` / `modenexus.enumerate_map()`
...
- **Learned priors.** ✅ *CPU EM done:* `CompiledSystem.fit_priors()`
  ...
  done too:* `modenexus.torch_learn.PriorLearner` /
...
- **Temporal diagnosis / mode tracking.** ✅ *First version done:*
  `modenexus.ModeTracker` maintains a beam-filtered belief over joint mode
