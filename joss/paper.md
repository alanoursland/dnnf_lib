---
title: 'modenexus: compiled DNNF reasoning, model-based diagnosis, and learning on one circuit'
tags:
  - Python
...
`modenexus` compiles finite-domain constraint theories into smooth
...
programming rather than system-model diagnosis. `modenexus` integrates
...
- `modenexus.fd`: finite-domain CNF, native d-way decision-DNNF compiler
...
- `modenexus.diagnosis`: `SystemModel` DSL (modes, sensors with
...
- `modenexus.tracking`: beam-filtered belief over joint mode assignments,
...
- `modenexus.torch_backend` / `modenexus.torch_learn`: layered batched
  ...
  model training.
- `modenexus.cnf` / `modenexus.nnf_io` / `modenexus.external`: DIMACS and c2d `.nnf`
