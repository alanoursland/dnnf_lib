...
- **GAP-032 — policy execution posterior carryover:** conditional planning
  results now create a public `BeliefPolicyExecution` through
  `result.execution(...)`. The execution state consumes concrete outcome and
  observation evidence, updates posterior belief on both terminal and
  nonterminal paths, routes the policy, tracks physical termination and
  accumulated action cost, and supports named robust outcome scenarios.
- **GAP-033 — one-step belief execution:** `BeliefPlanResult` now implements
  the same execution protocol for its selected action. Receding-horizon
  controllers can consume a concrete physical outcome, recover the updated
  latent posterior and action cost, and replan without constructing an
  observation policy or increasing the planning horizon.
...
- Updated the best-achievable counterexample and adaptive island controller
  ...
  planning.
- Updated the robust metric-scope, branch-safety, terminal-cost, regime
  phase-diagram, online-controller, and execution-posterior experiments to
  validate GAP-029 through GAP-033.
- Updated the black-box edge-case experiment and archived GAP-001 through
  GAP-033 under `modenexus_apps/gaps/fixed`.
